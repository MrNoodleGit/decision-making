...
class MCTSCostPolicy(CostPolicy):
    ...
    def calculate_expected_steps(self, node):
        ...
        '''
        try:
            samples_strings = self.graph.nodes[node]['sampled_cell_distances'].split(',')
        except:
            raise RuntimeError(f'{node}')
        samples = [int(sample) for sample in samples_strings]

        num_cells = int(self.graph.nodes[node]['new_observations'])
...
class SampleCellDistances(UCTSearch):
    ...
    def default_policy(self, leaf_node): # DONE
        ...
        while not self.terminal(current_node):
            ...
            # and append value to 'sampled_cell_distances'
            new_sample = self.sample_cell_distances(current_node)
            self.update_samples_attribute(current_node, new_sample)
    ...
    def update_samples_attribute(self, node, new_sample):
        '''
        Append new sample to 'sampled_cell_distances' attribute.
        '''
        try:
